# -*- coding: utf-8 -*-
"""
Sidemate — FastAPI + llama.cpp + Qwen3.5
版本号唯一权威来源：config.py 的 DEFAULTS["version"]（当前 0.9.7，前端/launcher 均从此处取）。
启动: python server.py
...
# 版本号唯一来源：config.py 的 DEFAULTS["version"]（当前 0.9.7）
        ...
        if _cfg_get("auto_warmup_llm", True):
            ...
            if _warmup_model:
                try:
                    ...
                    # P7-4: 用 OpenAI 兼容 /v1/chat/completions（替代 Ollama /api/chat）
                    import httpx as _hx
                    ...
                    _t0 = time.time()
                    _resp = _hx.post(
                        "%s/v1/chat/completions" % _base_url,
