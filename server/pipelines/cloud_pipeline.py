# -*- coding: utf-8 -*-
"""
pipelines/cloud_pipeline.py — 在线模式 SSE 管道（Agent Loop 版）
============================================================
管什么：所有 action 统一走 AgentLoop（FC 工具自主决策）
        chat/agent → 原生 agent；doc → agent_mode=doc 生成产物
不管什么：KB检索注入（Agent 自主调 search_kb）/ 并行融合（parallel 专属）
共享依赖：_base.StreamContext / _base.sse_event / _base._sanitize_output / doc_action / agent_loop
私有逻辑：上传文件预注入 / 模板解析 / AgentTimeline / 保存对话（都在本文件内）
============================================================
