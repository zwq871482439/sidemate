# -*- coding: utf-8 -*-
"""
doc_action.py — 文档生成 + 产物转换（local + cloud 共享模块）
============================================================
被谁用：local_pipeline（doc action 两阶段）+ cloud_pipeline/agent_loop（产物生成）
不被谁用：parallel_pipeline（不支持 doc action）
功能：两阶段文档生成 + docx/html/ppt 产物转换 + 文档文本读取
============================================================
两阶段流程：
两阶段流程：
        ...
        try:
            ...
            if kb_loaded:
                ...
                kb_context, kb_refs = kb.get_context(message, max_chars=2000, ai_mode="local",
                                                      actor="user", access_type="manual_cite")
                if kb_refs:
