...
_AGENT_BASE_PROMPT = (
    ...
    "系统已内置一套专业 CSS 组件，写 HTML 时直接用这些 class 即可获得精美排版：\n"
    "- 提示框：`<div class=\"note\">` `<div class=\"tip\">` `<div class=\"warning\">` `<div class=\"danger\">` `<div class=\"success\">`\n"
    "- 卡片：`<div class=\"card\">`（含可选 `<div class=\"card-title\">`）\n"
    "- 网格：`<div class=\"grid-2\">` `<div class=\"grid-3\">`（响应式，窄屏自动单列）\n"
    "- 统计：`<div class=\"stats\">` 内含 `<div class=\"stat\"><div class=\"stat-num\">99%</div><div class=\"stat-label\">完成率</div></div>`\n"
    "- 标签：`<span class=\"badge\">` `<span class=\"badge green/orange/red/gray\">`\n"
    "- 进度条：`<div class=\"progress\"><div class=\"progress-bar\" style=\"width:80%\"></div></div>`\n"
    "- 时间线：`<div class=\"timeline\"><div class=\"timeline-item\">...</div></div>`\n"
    "- 高亮：`<span class=\"highlight\">重点文字</span>`\n"
    "- 现代风格：在内容最外层加 `<div class=\"vibrant\">`（渐变背景+紫色标题，活泼风格）；默认是商务风格\n"
    "标准标签 h1/h2/h3/p/table/blockquote/code/pre/ul/ol/img 都已美化，直接用即可\n"
    "用户提风格要求时（如\"活泼点\"\"现代感\"），用 vibrant 风格或自己加 <style> 自定义\n\n"
    ...
    "PPT 最小结构示例（直接写 section，系统自动包装 reveal）：\n"
    "  <section><h1>标题页</h1><p>副标题</p></section>\n"
    "  <section><h2>内容页</h2><ul><li>要点1</li><li>要点2</li></ul></section>\n"
    "  <section>```mermaid ... ```</section>\n"
    "每页内容精简（演示用），一页一个主题，字号会自动放大\n\n"
    "### 格式判断规则\n"
