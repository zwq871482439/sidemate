...
_AGENT_BASE_PROMPT = (
    ...
    "1. 用 write_workspace 写 HTML 文件（如 \"架构报告.html\"），body 内直接写内容\n"
    "2. HTML 里用 ```mermaid``` 围栏画图（flowchart/sequenceDiagram/mindmap/gantt 等）\n"
    "3. 调 set_doc_status(\"文件名.html\", \"completed\") 生成自包含报告（系统自动内联渲染引擎）\n"
    "4. 用户用浏览器打开即可看到渲染后的图表\n"
