...
DOCX_PROTOCOL_PROMPT = (
    ...
    "   工作流（严格按序）：\n"
    "   ① create_docx(action=\"begin\", title=\"文档标题\") 开题；\n"
    "   ② create_docx(action=\"section\", deck=..., section=\"章节标题\","
    " content=\"markdown 正文\") 逐章提交——每章独立调用，正文是实质内容"
    "（支持 #/## 标题、- 列表、|表格|、**粗体**，不接受提纲占位）；\n"
    "   ③ 全部章提交完（通常 4-10 章）调 create_docx(action=\"build\", deck=...)"
    " 生成可下载 docx。\n"
    "   开做纪律：主题和章节结构至少一项明确就直接 begin——最多一次 ask 确认；"
    "资料不足按已有信息写，交付时说明假设。\n"
    "   正文写作要求：每章 300-800 字为宜（重点章可 1500+）；有依据才写具体数字；"
    "章末自然衔接下章。"
)
...
PPT_PROTOCOL_PROMPT = (
    ...
    "内容来源或风格细节，最多发一次 ask 确认（仅当关键信息完全缺失）；资料不足时"
    "按常识合理编排，并在交付时说明假设。\n"
    ...
    "   设计规则由 design_dna 选卡注入（默认 DNA-01 深蓝金；用户点名可换 DNA-02 素白极简 / DNA-03 暖砂杂志，以 begin 返回为准）：\n"
    "   - DNA-01 默认档：底色 #0F2B46（深蓝），强调色 #E8B54D（金），正文文字 #FFFFFF，次要文字 #B8C4D4\n"
    "   - 字号只能用所选卡的五档（工程门禁强制，超出 ±2 的档位出现 3 次即拒收）：\n"
    "     DNA-01 档位：页大标题/封面 72、副标题 34、正文 26、辅助说明 19、脚注/页码 18（DNA-02/03 各有档位，以工具返回为准）\n"
...
PWRITE_PROTOCOL_PROMPT = (
    ...
    "全部要写的文件后，输出 ask 围栏块（```ask```，输出格式不是工具）向用户"
    "列出计划，格式固定为 {\"question\":\"计划写入以下文件：…清单…（将覆盖的"
    "已有文件必须明确标注）\",\"options\":[\"同意，执行写入\",\"取消\"],"
    "\"allow_input\":false,\"kind\":\"plan_confirm\"}；用户点「同意」后系统会"
    "自动切到执行模式（你不用再调 set_exec_mode），直接用 project_write 逐个"
    "执行待执行计划里的写入，全部完成后调 set_exec_mode(\"plan\") 切回计划模式\n"
