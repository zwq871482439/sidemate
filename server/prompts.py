...
SYSTEM_PROMPT_V2 = (
    ...
    "3. 禁止续写用户消息，给出独立完整回答\n"
    "4. 超过3句必须编号分点，重点用**加粗**\n"
    "5. 你的真实能力：对话问答、知识库检索问答、联网搜索、生成Word文档(.docx)。\n"
...
KB_USER_PROMPT_TEMPLATE = (
    "根据【参考资料】回答问题。要求：\n"
    "1. 先给结论，再引用资料说明依据，在具体事实后标注来源编号，格式为「事实陈述 [1]」\n"
    "2. 完整复述原文的数字、时间和专有名词，不要省略\n"
    "3. 【最重要】严格基于参考资料作答，未提及的明确说「未提及」——不编造、不猜测、不引入外部知识，更不要凭空捏造百分比、数据、统计数字。每个事实必须有原文对应\n"
    "4. 追问时结合上文，不重复已回答内容\n\n"
    "【参考资料】\n{context}\n\n"
    "问：{question}\n答："
)
...
REFORMULATE_NO_HISTORY_PROMPT = """从以下用户问题中提取核心搜索关键词，用空格分隔。

规则：
- 提取问题中的核心概念、专有名词、关键术语
- 补充同义词或相关术语提升搜索召回
- 保持简洁，不超过 10 个词
- 输出只有一句话（空格分隔的关键词），不要解释，不要加引号
