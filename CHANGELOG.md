...
## [0.10.1] - 2026-09-24 — 发布收口：D2 绘图 + 技能管理 + 稳定性修复

### 新增
- **D2 绘图全链**：```d2 代码块前端渲染（官方 D2.js WASM 懒加载）；render_d2 工具
  服务端出 SVG 入工作区；create_ppt 支持 svg_file 引用（自动布局图直通原生 PPTX）；
  mermaid 兜底双渲染器并存（A/B 实测 d2 合法率 90% vs mermaid 85%）
- **技能页协议管理**：10 个协议（PPT/Word/代码执行/D2/任务编排/历史读取/项目写入/
  交付打包/卡片）技能卡可独立开关，关闭=工具与协议 prompt 一并撤除
- **离线AI 控制中心**（设置新子页）：模型卡片（使用中/未加载/运行占用）、使用/卸载、
  切换确认框含内存提示（推荐档位/吃紧警告）；闲置自动卸载显式化（默认 15 分钟，0=永不）
- **顶栏模型状态点**：绿=已载/空心=未载/转圈=加载中，点击直达离线AI页
- **搜索三级检索**：Bing → 年份降权改写 → 百度兜底（修复"2026年XX"查询跑偏到
  日历页）；Bing 标题解析改锚 h2（修复面包屑垃圾标题）
- **模型下载页联动**：已装模型一键「使用」
- 空会话治理：新建复用/切走自动清理（改过名保留）/启动存量清扫；默认名「新会话」

### 修复
- 离线模式静默无回复（三叠 bug：_loaded 门禁自愈/引擎路由显式化/IDLE-UNLOAD 假卸载）
- /api/status loaded 写死 True——改为运行时真相（引擎健康且正服务该模型）
- 消息队列不可达（生成中禁用输入框 → 保持可输入，Enter=排队）
- 刷新重附中断（log 未定义 ReferenceError 被吞）
- 技能 tab 死代码（createSkillsView 未挂载）；技能页标题隐形（误用深底 token）
- 输入框行数校准（3 行 66px/5 行 110px 封顶，第 6 行滚动）+ 内嵌滚动条
- 51 处原生 alert/confirm/prompt 全量替换为设计系统内嵌对话框
- 侧栏会话列表滚动位置跨 render/刷新保持
- composer pills 沉底（DeepSeek 布局）；思考 pill 迷你滚动条（类名撞车）
- 启动加载并行化 + 鱼骨骨架（侧栏/主区/KB/技能页）；假空态不再闪现
- create_ppt svg_file 宽容归一（R3：模型传 dict 报错）
- 标私密入口常显 + 在线态点出设计说明（R3 C1）
- mermaid 渲染空父竞态守卫（R3 次要观察）

## [0.10] - 2026-09-22 — 新版三栏界面 + Agent 能力 + 项目即文件夹（ prerelease ）

### 新增
- **新版三栏界面**（/newUI.html，与经典版并行互通）：左栏项目/会话 + 中栏对话 + 右视窗（会话/预览/文件/轨迹四 tab）；空状态 8 场景卡（在线）/3 卡（离线）；SVG 图标全面替代 emoji
- **项目即文件夹**：项目 = 一个文件夹（材料在根目录、AI 产物在 .sidemate/）；项目知识库（手动开关、向量化检索、仅在线）；跨项目引用；删除项目永不动磁盘文件
- **真 PPT**（create_ppt）：AI 逐页手写 SVG → 右视窗实时预览 → 同一份编译为可编辑的中文原生文本 PPTX（DNA-01 深蓝金设计系统）
- **M2 Agent 能力**：Harness 三机制（run_plan 调用计划 / spawn_reader 并行深读 / 工具注册表）、写权限计划/执行双模式（确认卡点同意才落盘 + 撤销 + 防穿越）、成果打包 deliver_package、调用轨迹时间线
- **记忆分层**：项目会话索引（热层）+「携」前情携带（选带层）+ read_session（冷层）；私密会话铁律（标为私密后不进索引/携带/读取）
- **启动不预载模型**：默认启动只起服务（启动快、后台不占资源），首条离线消息自动加载；闲置 30 分钟自动卸载；设置页可恢复启动即载
- **云端用量统计**迁入新版设置页（今日按小时/本周按天、分段柱状图、按模型、最近调用）
- **HTML 可视化报告应用内预览**（sandbox iframe）；mermaid DNA 主题换皮
- 新手引导补「新版界面与项目」步骤

### 修复
- PDF 摄入三级兜底链（fitz→pdfplumber→pypdf）+ 引用竞态守卫；上传文件预注入带文件名指纹（字数+修改时间）防相似材料混淆
- 目录选择器三重闸：粘贴路径未回车直接点 OK 正确注册、磁盘根目录拒绝注册、已是项目的文件夹直接切换
- 防澄清螺旋守卫（连续确认卡无推进时注入强提示）；场景 chip 跨会话泄漏；自动命名 markdown 星号清洗
- esbuild 构建后自动同步 bundle 指纹（根治"修复没生效"类浏览器旧缓存玄学）
- 代码质量批：死代码 30 函数 + 整模块 1 + config 死键 31 清理；引擎加载标记 5 处收口单点

---


### 生成架构解耦（M2）
- **生成任务化+事件日志**：pipeline 在后台线程跑，事件入缓冲+增量落盘——连接断（刷新）生成不死
- **刷新恢复**：gen-state/gen-live 端点+前端 EventSource 重附——DeepSeek 网页版同能力
- **多会话并行**：纯云模式跳过 GPU 队列+侧栏生成中脉冲标记
### skill 化设计+审计二批（M3）
- **通用产物管线 skill 挂载位**：任何 skill 可挂任何管线（设计域先行）
- **CSS 生成脚本**：DNA 卡→tokens 确定性生成
- **25 个无主端点清理**（审计二批）
### skill 平台+agent（M4）
- **code_exec**：受限子进程 Python 执行（沙箱方案 3）
- **Todo 任务步骤可视化**：AI 列步骤清单+视窗进度条
- **SKILL.md 解析器**：兼容行业格式，社区 skill 放 data/skills/ 即生效
- **MCP 客户端**：stdio JSON-RPC+工具发现+TOOL_REGISTRY 代理
- **定时任务**：daily/interval 调度+错过补跑
- **Artifacts 版本历史**：.sidemate/versions/ 备份在视窗可见
- **上下文用量透明化**：token bar 悬停五项明细
### 转正+发版（M5）
- **新版三栏界面转正为默认**（经典版挪 /classic.html，保留一个版本后移除）
- **v2 原生新手引导**（3 步浮层）
- 版本号 0.10
...
- **启动链路优化**：deps 检查两层化（启动只查存在性，深查归手动体检）+ 启动并行化（llama-server 提前拉起、KB 模型双线程加载）+ 进度活性检测替代固定超时
- **文件夹监视 + 自动同步** / **严格问答模式** / **引用连续性**（需评估）/ **图谱能力进阶**（待讨论）
- **已知问题待决**：FlagEmbedding 未预装导致 sparse 混合检索从未生效——099 决策装回或承认 dense-only
- KB 文档列表虚拟滚动（v0.9.8 顺延）

---

## [0.9.8] - 2026-08-18 — 云端双格式 + 知识星图 + 状态机重设计

### 新增
- **云端 API 双接口格式**（P8-1）：设置页可选 OpenAI 兼容 / Anthropic 格式；零新依赖 `anthropic_adapter`（httpx 直连 /v1/messages，对话 + 工具调用全支持）；CloudEngine 统一"归一化事件流"消费双格式
- **云端配置 UX 包**（P8-3）：拉取模型列表（下拉可选 + 手动输入兜底，上下文未知就地标注）、输入上限可编辑（自动预填/手填/覆盖，留空回落 32K）、保存成功持久横幅带"立即切换到在线模式"一键迁移、网络代理开关（系统代理/直连）、报错服务商原文透传
- **状态机重设计**（P8-6）：统一 `/api/app-state` 端点 + 前端 `AppState.derive()` 单点派生（锁卡五态/发送门禁/模式切换前置/KB 🔒 同源）；onboard 服务端标记为唯一权威；**引导式迁移**——已配云端但未加载模型时一键切换在线；E2E 27 项全绿
- **知识星图**（P8-9）：Obsidian 式文档关系图谱——k-NN 相似度连线（共享标签/同属分类标注连边原因）、力导向布局服务端预沉降（打开即静态图）、AI 聚类 + 子类拆分（≥6 篇拆 2 个子类，主/子类同色系）、点选浮条（关联知识列表 + AI 详解按需生成）、图例点击高亮聚焦、Ctrl+滚轮缩放/拖拽/双击复位
- **可视化报告增强**（P8-8）：图表选型指引（mermaid/统计图等样式库内置，HTML 报告拿去哪都能用）、交付前模型自查错（避免 mermaid 语法错交付）
- **联网搜索链路改造**（P8-2）：只搜不读护栏（未读页面收尾时注入提示）、工具预算提升（search 3→5 / fetch 5→15 / 轮次 20→26，每轮注入剩余预算）、KB 空库误搜修复、搜索→阅读→回答三段式工作流
- **CI**：GitHub Actions 工作流

### 优化
- **术语统一**：用户可见文案统一为离线/在线/并行体系（引擎实体=离线模型/在线服务；"本地"仅保留于数据驻留语境：数据不出本机/本地优先/本地知识库）
- **KB 体验**：洞察自动过期重生成（文档变动后标记 stale）、导入后自动刷新、聚类退化守护（LLM 合并成一类时按标签重建）、筛选改横排 chips（点一级展开二级）、侧栏折叠状态保持、标签 markdown 污染清理（`**` 等）
- **报告图表**：高度上限防撑爆、缩放手势与星图统一
- **工程**：envsetup 下载校验、懒加载加锁
- **卸载**：增加用户选择——模型/个人数据默认保留

### 修复
- **0.9.7 升级用户被误判未完成新手引导**（测试机实测回归）：0.9.8 起引导完成标记改为服务端文件唯一权威，老用户没有该文件 → 发消息被"请先加载模型"误拦（模型实际已加载）。现检测到使用痕迹（聊过天/配过云端/装过 LLM）自动补写标记，不再重新引导
- **云端调用挂死根治**（P8-7）：代理 CONNECT 瞬时 stall → httpcore 连接池锁同线程二次获取自死锁；LLM 事件源改 worker 线程 + 队列，120s 无事件丢弃客户端重建并抛可重试超时
- **流式中断处理**：60s 看门狗误杀覆盖已生成内容 → 报错改为追加不覆盖；终止标签三分（用户终止/超时/网络错误）；工具链/下载栏持久化白名单补齐（中断后不丢）
- **llama-server 静默死亡**：EXTERNAL 实例死亡不自愈 → 看门狗接管重启转 MANAGED
- **启动窗口 502**：llama-server 加载滞后于 HTTP 就绪 → 本地引擎连接重试 3 次
- **流式回复尾巴丢失**：done 补刷判据被时间戳文本抵消 → 改用已渲染长度精确对比
- **并行模式自 0.9.7 起静默失效**（回归）：0.9.7 安全审计收紧配置校验时 `ai_mode` 合法值漏掉 `parallel`，切换并行被静默拒绝（UI 显示已切换、实际请求回落在线）。校验器补全，并行管道恢复真实运行
- **聊天引用的文档刷新后标记丢失**：三层修复——① 新会话首条消息引用在自动建会话时被清（快照提前到 newChat 之前）；② AI 完成后后端重建消息覆盖前端落盘的引用标记（save_chat 增加末尾继承，按前缀匹配兼容历史）；③ 前端往消息内容追加的引用尾巴与后端重建文本不一致导致继承失配（删除多余尾巴，文档注入由 file_path 单一通道承担）
- **附件引用状态跨会话残留**：新建/切换会话统一清理（pendingFile/_refFilePath/已上传列表/KB 多选），防止 A 会话的引用被带进 B 会话首条消息；静态资源改内容指纹缓存（发版脚本自动 bump，根治手动 `?v=` 忘更新）
- **标签太散（P0，测试机实测）**：小模型打标产出大量单篇近义变体分类（如「中医养生学/中医药道养生/中医养生气血经络」各成一类）；三层修复——LLM 归并结果确定性后处理（bigram 相似度收敛+「其他」兜底）、归并 prompt 两级化（一级大类≤5+原标签为二级）、侧栏分类>6 时前端同款算法归并渲染（一级大类/点选展开二级原分类，过滤 OR 匹配）
- **批量上传部分丢失（测试机实测）**：模型未就绪时连传多文件，前几个被 400 静默拒绝；现遇"模型未加载"立即中断批量并提示"已暂停，剩余 N 个稍后重传"
- **气泡模式左右分布**：用户消息靠右（≤80% 宽）、AI 靠左（≤92%），经典聊天布局
- **星图子类名显示原始字典**：小模型拆分类返回 {name,desc} 对象时 str(dict) 整串进子类名（图例出现 {'name': ...}）——dict 取 name，异常解析走标签兜底；AI 详解 prompt 防幻觉引用（实测出现"就像你提到的…"）

### 已知限制（prerelease）
- 0.8B 档模型的文档摘要偏短、偶有复读指令（模型能力局限，4B 档显著改善；后续版本评估摘要链路针对性优化）
- 冷启动约 60-90 秒（faiss/torch 依赖加载）——0.9.9 启动链路优化专项处理
- config 并发写丢更新

---

## [0.9.7] - 2026-08-03 — llama.cpp 底座 + 安全审计 + 全新安装体验

### 底座与架构
- 推理底座从 Ollama 切换为 **llama.cpp（llama-server）**，Go Launcher 进程管理 + 看门狗重构
- 产品定位明确：本地优先的 AI 桌面应用（模型运行平台，非 AI 提供方）
- 法律合规：THIRD-PARTY-NOTICES 补全、品牌名中性化、移除绝对化用语

### 新增
- **快速开始**：下载页按内存自动推荐模型档位（16GB→0.8B / 24GB→2B / 32GB→4B），一键下载 LLM + 知识库组合，自动接续下载
- **下载即可用**：LLM 下载完成自动注册扩展、自动加载模型并后台预热，无需手动切换
- **KB Tab 状态锁**：知识库 Tab 常显，未装模型显示 🔒 徽标 + 引导页
- **环境检查三层**：启动依赖检查 + 环境指纹 + 依赖清单
- **CPU/GPU 切换**：设置页可切换推理设备，切换后自动重载模型
- 云端配置页标注接口格式（OpenAI 兼容）
- 上下文压缩优化、reformulate 全走 LLM、KB 引擎修复

### 安全（审计修复 15 项）
- 高风险端点本地来源校验（Origin 检查）、CORS 危险开关二次确认
- API Key 接口脱敏返回、配置项校验 + 原子写入
- pip 安装白名单（requirements 清单）、扩展包 SHA256 校验
- SSRF 防护（URL 分类 + 逐跳重定向检查）、路径穿越防护、工作区文件锁
- KB 检索错误不再回传堆栈、会话删除/文件删除鉴权

### 修复（打包与全新安装体验）
- 安装包补打 3 档模型 meta.json（新装机下载页为空）、logo.ico 入包
- Windows 英文系统 cp1252 编码导致启动崩溃
- 卸载残留（data/python/server 未清理）；卸载增加用户选择——模型/个人数据默认保留
- 启动器退出连坐杀浏览器（Job Object 浏览器进程脱离）
- 全新安装全链路：对话误判"暂无可用模型"（frozen global + `_loaded` 未同步双门禁）、
  switchTab 无 btn 崩溃致全部跳转失效、KB Tab 被误隐藏、onboard 与状态锁叠加冲突、
  欢迎页选项卡纯装饰不可点
- 下载体验：速度计算错误（速度=累计量）、快速开始队列断链（按两次才续）、
  KB 进度错乱（4.2/2.xGB）、完成事件先于模型加载
- 模型/云端状态多源不一致系统性修复（卸载后误判已加载、设备切换重载传参错误、
  配完 Key 当次会话被误拦、发送门禁陈旧）
- 看门狗日志降频（状态变化才记录）、静态资源版本号缺失致浏览器吃旧缓存

### 工程
- `envsetup.py` 一键部署替代 setup_dev.bat
- 新增测试：安全修复验证 24 项 + 纯逻辑 25 项，回归 145 项全绿
- C:\testenv 发版质量门禁：安装包冒烟检查 + 全新状态模拟器 + 全新状态 E2E 21 项
