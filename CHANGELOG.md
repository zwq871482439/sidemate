...
主线：**云端体验 + 联网质量**（P8-1/2/3）+ 工程小修（P8-4）。

- **P8-1 云端 API 双接口格式** ✅ 已实现（2026-08-04）：设置页可选 OpenAI 兼容 / Anthropic 格式；config 新增 `cloud_api_format`；`core/anthropic_adapter.py`（httpx 直连 /v1/messages，零新依赖，对话 + 工具调用全支持）；CloudEngine 统一"归一化事件流"消费，OpenAI/Anthropic 双格式分发
- **P8-2 联网搜索链路改造**（v0.9.7 用户反馈）：① 只搜不读加护栏（search 后未 fetch 就收尾时注入一次提示）；② 次数 search_web 3→5 / fetch_url 5→15 / MAX_ROUNDS 20→26，每轮注入剩余预算快照；③ 修复 KB 空库仍误搜（注入条件加文档数>0，prompt 引导动态裁剪）；④ 在线 prompt 补搜索→阅读→回答三段式工作流、来源必须出自读过的页面
- **P8-3 云端配置 UX 包** ✅ 已实现（2026-08-04）：接口格式下拉 + 拉取模型列表（下拉可选 + 手动输入兜底，上下文未知就地标注）、输入上限可编辑（自动预填/手填/覆盖，留空回落 32K）、保存成功持久横幅（带"立即切换到在线模式"一键迁移）、测试/拉取报错服务商原文透传、配置加载中禁用保存
- **P8-6 状态机制重设计** ✅ 已实现（2026-08-04）：统一 `/api/app-state` 端点 + 前端 `AppState.derive()` 单点派生（锁卡五态/modelTag/发送门禁/模式切换前置/KB 🔒 同源）；onboard 服务端标记为唯一权威（localStorage 降级缓存）；**引导式迁移**——锁卡B"你已配置云端，点这里切换"一键切换；E2E 27 项全绿（`testenv/test_app_state_e2e.py`）
- **P8-5 公安备案官方图标**（网站）：页脚公安备案按官方要求加备案图标（图标在前编号在右）
- **术语统一** ✅（2026-08-04，用户反馈）：用户可见文案统一为**离线/在线/并行**体系——模式=离线/在线/并行，引擎实体=离线模型/在线服务（API）；数据驻留语境保留"本地"（数据不出本机/本地优先/本地知识库）；只改文案不动代码标识（local/cloud），前端 60+ 处 + 服务端 toast 1 处
- **P8-4 工程小修**：KB 文档列表虚拟滚动、envsetup.py 下载 SSL+SHA256 校验、懒加载单例线程安全
- ~~DPAPI 加密 API Key~~（取消，行业无此惯例）；测试机反馈 buffer 随时修
