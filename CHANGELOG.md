...
- **P8-1 云端 API 双接口格式**：设置页可选 OpenAI 兼容 / Anthropic 格式（v0.9.7 测试机反馈：配置页未标注格式。v0.9.7 已加"OpenAI 兼容"标注；下版本 config 新增 `cloud_api_format`，云端引擎按格式分发，Anthropic 走 `/v1/messages` + SSE 事件适配器）
...
- **P8-3 云端配置 UX 包**（测试机反馈，已查实）：保存竞态禁用、保存后回读+带值 toast、云端 400 报错透传、拉取模型列表按钮（下拉+可手输，根治模型名大小写写错）
- **P8-6 状态机制重设计**（测试机两轮状态 bug）：统一 `/api/app-state` 端点 + 前端派生渲染；onboard 以服务端标记为唯一权威（重装必重新引导）；**引导式迁移**——配置完成给一键切换引导（"你已配置云端，点这里切换"），不静默自动切换
