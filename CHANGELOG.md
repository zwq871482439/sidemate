...
- **P8-2 联网搜索链路改造** ✅ 已实现（首批完成，2026-08-05 核实代码四项全落地）（v0.9.7 用户反馈）：① 只搜不读加护栏（search 后未 fetch 就收尾时注入一次提示）；② 次数 search_web 3→5 / fetch_url 5→15 / MAX_ROUNDS 20→26，每轮注入剩余预算快照；③ 修复 KB 空库仍误搜（注入条件加文档数>0，prompt 引导动态裁剪）；④ 在线 prompt 补搜索→阅读→回答三段式工作流、来源必须出自读过的页面
...
- **P8-4 工程小修**：envsetup 下载校验 ✅ / 懒加载加锁 ✅ / KB 文档列表虚拟滚动（未做，下版本评估）
