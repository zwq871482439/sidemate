...
### 新增
- **预览中心（视窗重构）**：预览改为「文件」的下一层（drill-down）——面包屑返回、
  顶栏（文件名/打开位置/下载）；按扩展名注册表渲染：pptx 逐页 SVG、html 报告
  sandbox iframe、docx 章节回放（注明近似排版）、svg 内联、md 渲染、图片
- **聊天产物卡片**：AI 生成文件后显示预览卡片（类型图标/名称/元信息/预览/下载），
  点卡片即开视窗预览；替换旧小下载标
- **思考过程可视化三层**：胶囊摘要行（运行态呼吸点）→ 阶段台账（检索与阅读/
  生成产物分组、×N 折叠计数、失败保留、组头小计）→ 思考轮原文折叠；
  轨迹 tab 同构重设计（步骤/总耗时/轮次 + 每轮台账）；0.10.1 老会话兼容回放
### 修复
- 视窗 tab 点击后高亮不随内容切换（B1，head 不重渲染）
- 知识库「卡片/列表」切换图标不可见（裸 SVG 无尺寸约束）
- 「停止」按钮偶现残留（onDone 复位加固 + 快照拉取保底）
### 变更
- 「上传文档」→「添加文档」（本地语义，避免"传到网络"观感）
- 经典版界面退役（/classic.html 移除，入口拆除，/ 重定向新版）
- 官网演示同步脚本 tools/sync_demo_bundle.py（发版一键同步 bundle+指纹）
