...
> - **隐私混合模式**（P6 新增）：独立的第三种模式，不是替代在线模式
> - **两者并存**：在线模式用令牌，隐私混合用脱敏草稿，是两个独立模式不是迭代关系
> - **KB → 知识库改造**：原"文库"功能拆分，对话功能迁出到隐私混合模式，KB 专心做档案管理
...
| **🛡️ 隐私混合模式**（名字待定）| **脱敏草稿**（P6 做）| 想用云端又怕泄露 | P6 新增 |

**关键澄清**：
- ❌ **不是**"P5 过渡 → P6 终极替代"
- ✅ **是**"在线模式永久用令牌，隐私混合是新加的独立模式"
- 用户可以按场景选：纯本地 / 信任云端 / 想要云端能力但怕泄露

**KB（文库）→ 知识库 改造**：
- 原"文库"= 档案管理 + 对话（混在一起）
- 改造后 = **纯档案管理**（上传/打标/聚类/检索测试/隐私标记）
- 对话功能迁出到 Chat Tab 的"🛡️ 隐私混合模式"
- 知识库可能扩展：知识自动关联等（P6+ 考虑）

**P5 vs P6 分工**：
- P5 做：令牌系统 + 私密文档 + chunk 切小 + 检索统一（在线模式强化）
- P6 做：隐私混合模式 + KB→知识库改造 + 模式系统重构
...
| **⚠️ 模式定位澄清** | 三模式并存非替代：本地/在线(令牌)/隐私混合(草稿)。令牌系统是在线模式的永久组件，不是过渡方案 | 2026-06-19 |
| **⚠️ 文库→知识库** | 原"文库"拆分：对话迁到 Chat Tab 隐私混合模式，知识库 Tab 改纯档案管理（上传/打标/聚类/检索测试/隐私标记）| 2026-06-19 |
