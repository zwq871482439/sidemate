...
> ✅ **本文档随 0.9.6 首发重发**（架构已在 0.9.6 完整发版）。
>
> 版本：v0.9.6（当前发版） / 目标：v0.9.7
...
│   ├── step_model.py       # 步骤流数据模型 (v0.9.6)
│   ├── access_token.py     # 令牌访问控制 (v0.9.6)
│   ├── thread_pool.py      # 线程池 (v0.9.6)
│   └── batch_queue.py      # 批量任务队列 (v0.9.6)
├── pipelines/              # 管道层
│   ├── _base.py            # 基础类型
│   ├── local_pipeline.py   # 本地管道
│   ├── cloud_pipeline.py   # 云端管道
│   ├── parallel_pipeline.py # 并行融合管道 (v0.9.6)
│   ├── compare_pipeline.py # 对比管道
│   └── doc_action.py       # 文档生成两阶段 (v0.9.6)
...
### 4.7 步骤流模型 (core/step_model.py) （v0.9.6 新增）
...
> 详见 [v0.9.6-step_model设计.md](v0.9.6-step_model设计.md)

### 4.8 访问令牌 (core/access_token.py) （v0.9.6 新增）

三级令牌访问控制（full/search/none），仅存内存。私密文档标记持久化到 `kb_meta.json`。

### 4.9 并行管道 (pipelines/parallel_pipeline.py) （v0.9.6 新增）

本地检索 KB + 云端通用知识独立回答，本地自动融合。KB 原文不离开本机。

### 4.10 文档生成管道 (pipelines/doc_action.py) （v0.9.6 新增）
