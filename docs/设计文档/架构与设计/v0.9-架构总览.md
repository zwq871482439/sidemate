...
> 版本：v0.9 Patch 3
> 技术栈：Python 3.14 + FastAPI + Ollama (Vulkan) + Qwen3.5-4B
> 部署模式：纯内网离线 / 可选云端增强
> 日期：2026-06-10
...
graph TB
    subgraph Frontend["前端 (原生 HTML/CSS/JS)"]
        UI_CHAT["Chat Tab<br/>对话界面"]
        UI_KB["KB Tab<br/>知识库管理 + 对话"]
        UI_SETTINGS["Settings<br/>设置页面"]
    end
...
│   └── generate_queue.py   # 队列基础（LLMScheduler 前身）
├── pipelines/              # 管道层
│   ├── _base.py            # 基础类型（EngineResult, StreamContext, sse_event）
│   ├── local_pipeline.py   # 本地管道
│   ├── cloud_pipeline.py   # 云端管道
│   └── compare_pipeline.py # 对比管道（本地∥云端→融合）
├── routers/                # API 路由层（5 个 Router）
│   ├── chat.py             # 对话 API（SSE 流式）
│   ├── kb.py               # 知识库管理 API
│   ├── settings.py         # 设置 API
│   ├── action.py           # Action 扩展 API
│   └── recorder.py         # 录音转写 API
├── session/                # 会话管理
│   └── chat_store.py       # 对话历史持久化
├── knowledge_base.py       # 知识库核心（嵌入、检索、精排）
├── config.py               # 集中配置管理
