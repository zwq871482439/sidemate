...
>
> 版本：v0.9.6 | 文件：`server/routers/kb.py`（`api_kb_overview_refresh`）/ `server/static/js/qa.js`（`_kbRenderInsightDashboard`）
...
**P6 修复**：旧版 category 为空时直接喂空数据给 LLM，导致瞎编。v0.9.6 fallback 到 `doc.tags` 收集。
...
**v0.9.6 改进**：旧版「追问」嵌在 insight prompt 里，LLM 容易给出「请分享更多内容」这类空泛问题。改为**独立 LLM 调用**，追问紧扣文库内容。
...
.kb-dash-ask {
  ...
  font-size: 13px;
  line-height: 1.5;
  overflow-wrap: break-word;   /* v0.9.6 修复 */
  word-break: break-word;       /* v0.9.6 修复 */
