...
>
> 版本：v0.9.6 | 文件：`server/static/js/welcome-tour.js` / `server/index.html`
...
**v0.9.6 修复**：旧版用 `[data-tab="chat"]` 选择器匹配不到实际 `onclick="switchTab('chat',this)"` 形式的 tab 按钮，导致切 Tab 失败。
...
**v0.9.6 修复**：KB Tab 等可滚动 Tab 中目标可能在视口外（`getBoundingClientRect` 的 top 很大或为负），先 `scrollIntoView` 让它进入视口再定位。

### 4.5 Spotlight 镂空（v0.9.6 核心修复）
...
**v0.9.6 修复**：旧版箭头恒在 `left:50%`（卡片中心），目标偏离卡片中心时指偏。
...
<div class="settings-section">
  <h4>程序版本</h4>
  <div class="version-display" id="versionDisplay">v0.9.6</div>
