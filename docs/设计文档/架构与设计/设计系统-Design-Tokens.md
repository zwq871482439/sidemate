...
> ⚠️ **实现差异说明**（v0.9.7 更新）
>
> 本文档定义的 token 以**琥珀 `--accent-default: #c9976c`** 为主 CTA 色，
> 这是覆层/空状态场景的**理想设计参考**。
>
> 但实际代码实现（`server/static/css/main.css`）中：
> - 主 CTA 色是**深蓝** `--accent-color: var(--primary-700)` = `#2d4a6f`
> - 琥珀 `--accent-600: #c9976c` 仅用于 Logo 和极少量装饰
>
> **v0.9.7 决策：保持代码深蓝主调不变**（视觉统一性 + 改动成本）。
> 后续如需切换到琥珀主调，需同步修改 main.css 约 15-20 处 CSS 变量。
