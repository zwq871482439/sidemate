...
> 版本：v0.9 Patch 4 | 日期：2026-06-11 | 状态：🔄 更新中（v4 — 增加产品化属性）

## 一、总览

Patch 4 以**代码重构 + 数据管理 + 依赖安全 + 首次引导 + 产品化提升**为主线，不新增核心功能，专注提升可维护性、健壮性、用户体验和产品专业感。
...
### Batch 4：收尾 + 扩展注册 + 产品化基础

**目标**：ISS 脚本适配 + LLM 包纳入扩展注册 + 基础产品化元素。
...
| 冒烟测试 | 全新安装 + 覆盖安装 + 卸载清理 |
| **ISS EULA 页** | 安装向导中增加 LICENSE 展示页（Inno Setup `InfoBeforeFile`） |
| **ISS 品牌图** | 安装向导自定义 WizardImage/WizardSmallImage（左侧品牌图 + 顶部小图） |
| **LICENSE 文件打包** | LICENSE + THIRD-PARTY-NOTICES 纳入安装目录，设置页可查看 |

### Batch 5：首次使用引导 + 多状态审查 + 关于对话框

**目标**：根据用户安装状态展示分层的首次使用引导 + 验证 6 种状态组合下程序行为正确 + "关于"对话框。
...
设置页增加"关于"入口，点击弹出模态框，展示：

| 内容 | 来源 |
|------|------|
| 产品 Logo | static/img/logo.svg |
| 产品名称 + 版本号 | `GET /api/settings` → version_display |
| 构建日期 | server.py 编译时写入（可选） |
| 版权声明 | "© 2026 许清楚. All rights reserved." |
| 第三方许可 | 链接到 THIRD-PARTY-NOTICES 文件内容（或新 Tab 展示） |
| 运行环境信息 | Python 版本 / Ollama 版本 / 模型状态 / GPU 信息 |

**实现方式**：settings.js 中新增 `showAbout()` 函数，复用现有模态框样式。后端 `/api/system/info` 已有基础信息，可能需补充 Ollama 版本。

#### 5.10 版本号展示优化

| 改动 | 说明 |
|------|------|
| 设置页标题栏 | "桌伴 Sidemate" 后追加版本号，如 "v0.9 Patch 4" |
| 窗口标题 | `document.title` 改为 "桌伴 v0.9"（简洁版） |
| 版本来源 | 统一从 `/api/settings` 获取，不再硬编码 |
...
| Batch 4 | 1-2 天 | ISS + 扩展注册 + EULA + 品牌图 + 许可打包 |
| Batch 5 | 1-2 天 | 引导 UI + 多状态审查 + 关于对话框 + 版本号优化 |
| **合计** | **6-9 天** | |
...
- **LICENSE + THIRD-PARTY-NOTICES 随安装包分发**，设置页"关于"中可查看
- **ISS EULA 页**展示闭源专有许可，用户需同意后才可安装
